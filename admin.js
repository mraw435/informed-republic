...
function getBriefs() {
  try { return JSON.parse(localStorage.getItem(STORAGE_KEY_BRIEFS) || '{}'); }
  catch { return {}; }
}
function saveBriefs(briefs) {
  localStorage.setItem(STORAGE_KEY_BRIEFS, JSON.stringify(briefs));
}
...
function checkExistingBrief() {
  ...
  if (existing) {
    note.textContent = `A ${existing.status} brief already exists for this date.`;
...
document.getElementById('genDate').addEventListener('change', checkExistingBrief);
...
function showExistingDraft(date, brief) {
  currentDraftDate = date;
  currentDraftHTML = brief.html;
  document.getElementById('genProgress').style.display = 'none';
  showDraft(date, brief.html);
  document.getElementById('feedbackBox').value = brief.feedback || '';
}
...
function publishBrief() {
  ...
  document.getElementById('draftArea').style.display = 'none';
  document.getElementById('genProgress').style.display = 'none';
  showToast('✓ Brief published! It\'s now live on the homepage.', '#1e8449');
  currentDraftDate = null;
  checkExistingBrief();
}
...
function renderQueue() {
  ...
  const pending = Object.entries(briefs)
...
function quickPublish(date) {
  const briefs = getBriefs();
  if (!briefs[date]) return;
  briefs[date].status = 'published';
  briefs[date].publishedAt = new Date().toISOString();
  saveBriefs(briefs);
  showToast('✓ Published!', '#1e8449');
  renderQueue();
}
...
function renderArchive() {
  ...
  const published = Object.entries(briefs)
  ...
  list.innerHTML = published.map(([date, brief]) => `
    <div class="archive-item">
      <span class="queue-item-date">${formatDisplayDate(date)}</span>
      <span class="queue-item-preview">${stripHTML(brief.html).slice(0,120)}…</span>
      <div class="queue-item-actions">
        <span class="status-badge status-published">Published</span>
        <button class="btn-secondary btn-sm" onclick="reviewFromQueue('${date}')">Edit</button>
      </div>
    </div>
  `).join('');
}
